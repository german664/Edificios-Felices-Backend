...
def nuevo_residente(id):
    if request.method == 'POST':
        ...
        if not edificio:
            ...
            departamento = DepartamentoUsuario.query.filter_by(
                numero_departamento=numero_dpto, edificio_id=id).first()
    ...
    if request.method == 'GET':
        residentes = NuevoResidente.query.filter_by(
            edificio_id=id, estado=False).all()
        if not residentes:
            return jsonify({"msg": "No hay residentes nuevos por agregar"}), 404
        else:
            residentes = list(
                map(lambda residente: residente.serialize(), residentes))
